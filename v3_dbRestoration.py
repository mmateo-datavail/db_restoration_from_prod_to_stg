...
Usage: python v3_dbRestoration.py <json_file_path> <snapshot_arn> <region> <secret_arn> <secret_key's_username> <secret_key's_password> <new_db_identifier> [<db_cluster_name>]
For Aurora, <new_db_identifier> is used as the regional cluster identifier and [<db_cluster_name>] is used as the writer instance identifier.
"""

# # RDS regular with MySQL
# venv/bin/python v3_dbRestoration.py mysql_metadata.json arn:aws:rds:us-west-2:123456789012:snapshot:mysql-snap us-west-2 arn:aws:secretsmanager:us-west-2:123456789012:secret:mysql-pass-abc123 "username" "password" mysql-restored

# # Aurora PostgreSQL
# venv/bin/python v3_dbRestoration.py aurora_pg_metadata.json arn:aws:rds:us-west-2:123456789012:cluster-snapshot:aurora-pg-snap us-west-2 arn:aws:secretsmanager:us-west-2:123456789012:secret:aurora-pass-def456 "username" "password" aurora-instance-restored aurora-cluster-restored

# # Oracle
# venv/bin/python v3_dbRestoration.py oracle_metadata.json arn:aws:rds:us-west-2:123456789012:snapshot:oracle-snap us-west-2 arn:aws:secretsmanager:us-west-2:123456789012:secret:oracle-pass-ghi789 "username" "password" oracle-restored
...
class RDSRecreator:
    ...
    def extract_aurora_cluster_info(self, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            info = {
                ...
                'availability_zones': self.safe_get(db_cluster, 'AvailabilityZones', [])
            }
    ...
    def wait_for_resource(self, resource_type: str, resource_name: str, max_attempts: int = 1000, require_reset: bool = False) -> bool:
        """Wait for a resource to become available with improved error handling"""
            ...
            try:
                if resource_type == 'cluster-parameter-group':
                    ...
                    if status == 'resetting-master-credentials':
                        ...
                        if require_reset:
                            if seen_resetting:
                                logger.info(f"DB instance '{resource_name}' has completed resetting and is available")
                                logger.info(f"DB instance '{resource_name}' is available; proceeding without waiting for a subsequent resetting-master-credentials state")
                                return True
                            logger.info(f"DB instance '{resource_name}' is available but has not yet passed through resetting-master-credentials; waiting...")
                            return True
                        else:
    ...
    def _apply_aurora_post_restore_modifications(self, instance_identifier: str, cluster_identifier: str, db_info: Dict[str, Any], master_password: Optional[str] = "password") -> bool:
        ...
        try:
            ...
            if modifications_needed:
                ...
                # Ensure MasterUserPassword and other settings are applied
    ...
    def recreate_rds_instance(self, json_file_path: str, snapshot_arn: str, new_db_identifier: str, secret_arn: str,
                              ...
                              db_cluster_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        logger.info(f"DB Cluster Name: {db_cluster_name or 'N/A'}")
        ...
        try:
            ...
            if is_aurora:
                ...
                cluster_identifier = db_cluster_name or f"{new_db_identifier}-writer"
                ...
                if db_cluster_name:
                    logger.info(f"Ignoring DB cluster name for regular RDS instance restoration: {db_cluster_name}")
...
def main():
    ...
    if len(sys.argv) not in (8, 9):
        print("Usage: python v3_dbRestoration.py <json_file_path> <snapshot_arn> <region> <secret_arn> <username_secret_key> <password_secret_key> <new_db_identifier> [<db_cluster_name>]")
        print("Example: python v3_dbRestoration.py rds_metadata.json arn:aws:rds:us-east-1:123456789012:snapshot:mydb-snapshot us-east-1 arn:aws:secretsmanager:us-east-1:123456789012:secret:rds-secret username password mydb-restored")
        print("Example (Aurora): python v3_dbRestoration.py aurora_metadata.json arn:aws:rds:us-east-1:123456789012:cluster-snapshot:aurora-cluster-snapshot us-east-1 arn:aws:secretsmanager:us-east-1:123456789012:secret:aurora-secret username password aurora-instance-restored aurora-cluster-restored")
        sys.exit(1)
    ...
    new_db_identifier = sys.argv[7]
    db_cluster_name = sys.argv[8] if len(sys.argv) == 9 else None
    
    try:
        recreator = RDSRecreator(region)
        result = recreator.recreate_rds_instance(
            json_file_path, snapshot_arn, new_db_identifier, secret_arn, username_secret_key, password_secret_key, db_cluster_name
        )
